...
function openEmergencyPowerRecord() {
  ...
  showModal(modalFrame({ code: "EVIDENCE SECURED · EMERGENCY POWER", title: "비상 전원 기록", body: `<article class="power-record"><header><small>3F 격리실 단말기 · 로컬 진단 출력</small><strong>통신 차단 및 비상 전원 현황</strong></header><dl class="power-audit"><div><dt>외부 송신</dt><dd>차단됨</dd></div><div><dt>차단 명령 서명</dt><dd>HXB · 원격 관리 계정</dd></div><div><dt>내부 인터폰</dt><dd>별도 회선 · 정상</dd></div><div><dt>독립 비상 송신기</dt><dd>주 통신망과 분리 · 전원 대기</dd></div></dl><h3>1F 로비 · 비상 배전반</h3><p>남은 가용 출력: <strong>8칸</strong></p><div class="power-loads" aria-label="각 장치에 필요한 전력"><div><span>비상 송신기</span><strong>4칸</strong></div><div><span>백신 후보 냉장 보관 장치</span><strong>3칸</strong></div><div><span>격리문 개별 개방 구동부</span><strong>5칸</strong></div></div><p class="power-handwriting">“차단 원인과 전원 상태를 같이 남긴다. 내 계정으로 지운 출입 기록도 함께 제출해 줘.”</p><p>전원을 배분하기 전 배전반의 현장 상태와 안전 조건을 다시 확인해야 한다. 이 기록에는 전환 순서가 적혀 있지 않다.</p></article><div class="vaccine-access"><small>다음 단서 · 로비 비상 배전반</small><strong>기록을 증거 인벤토리에 보관했다.</strong><p>전력 배분 퍼즐은 다음 업데이트에서 이어집니다. 지금은 로비로 돌아가거나 이전 증거를 다시 조사할 수 있습니다.</p></div><button class="primary-button material-check-button" type="button" data-power-return-lobby>로비로 돌아간다 <span>→</span></button>` }));
  $("#modal").classList.add("evidence-modal", "exploration-modal", "isolation-modal");
